...
public class Coolsms extends Https {
	...
	public JSONObject sent(HashMap<String, String> params) {
		...
		try {
			...
			response = https.request(sms_url + "sent", params); // GET?„?»ë«—ï¿½ë»? ? ?Œ?ˆ? ?ˆê½?	
		} catch (Exception e) {
			response.put("status", false);
			response.put("message", e.toString());
		}
	...
	public JSONObject cancel(HashMap<String, String> params) {
		...
		try {
			...
			// Cancel? ?™?˜™ response ?¶?‰?˜™ empty ç­Œë¡³?˜™ ? ?„?‰ï¿½â‘¨?˜™
	...
	public JSONObject balance() {
		...
		try {
			...
			// GET?„?»ë«—ï¿½ë»? ? ?Œ?ˆ? ?ˆê½?	
			response = https.request(sms_url + "balance", params); // GET?„?»ë«—ï¿½ë»? ? ?Œ?ˆ? ?ˆê½?	
		} catch (Exception e) {
			response.put("status", false);
			response.put("message", e.toString());
		}
